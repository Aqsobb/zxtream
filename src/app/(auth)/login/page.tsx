  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        if (data.success) {
          userData.displayName = data.data.displayName || user.displayName;
          userData.photoURL = data.data.photoURL || user.photoURL;
          (userData as any).role = data.data.role || 'member';
          (userData as any).isOwner = data.data.isOwner || false;
          (userData as any).totalExp = data.data.totalExp || 0;
          (userData as any).level = data.data.level || 1;
          localStorage.setItem('user', JSON.stringify(userData));
        }
      } catch {}
          ...
          <form onSubmit={handleEmailLogin} className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2">
                <input type="checkbox" className="w-4 h-4 rounded border-dark-600 bg-dark-800 text-primary-600 focus:ring-primary-500" />
